...
class Graph extends LazyLogging {
  ...
  private val nodeLabelsIndex = mutable.Map[Label, mutable.Set[ID]]().withDefaultValue(mutable.Set.empty[ID])
  private val outgoingRelationships = mutable.Map[ID, mutable.Map[String, mutable.Set[ID]]]().withDefaultValue(mutable.Map.empty.withDefaultValue(mutable.Set.empty))
  private val incomingRelationships = mutable.Map[ID, mutable.Map[String, mutable.Set[ID]]]().withDefaultValue(mutable.Map.empty.withDefaultValue(mutable.Set.empty))
   ...
   * 
   * @param labels Set of labels to assign to the node
   * @param properties Map of properties to assign to the node
   * @return The newly created node
   */
  def createNode(labels: Set[String] = Set.empty, properties: Properties = Map.empty): GraphResult[Node] = {
   ...
   * @return An Option containing the node if found
   */
   ...
   * 
   * @param id The node ID
   * @param labels New set of labels (completely replaces existing ones)
   * @param properties New map of properties (completely replaces existing ones)
   * @return GraphResult containing the updated node or a failure
   */
   ...
   * 
   * @param id The node ID
   * @return GraphResult containing the deleted node or a failure
   */
   ...
   * 
   * @param propertyKey The property key to match
   * @param propertyValue The property value to match
   * @return Set of nodes with matching property values
   */
   ...
   */
  def createRelationship(sourceId: ID, targetId: ID, relationType: String, properties: Properties = Map.empty): GraphResult[Relationship] = {
    ...
    // Update indexes
    val sourceOutgoing = outgoingRelationships.getOrElseUpdate(sourceId, mutable.Map.empty.withDefaultValue(mutable.Set.empty))
    val relTypeSet = sourceOutgoing.getOrElseUpdate(relationType, mutable.Set.empty[ID])
    relTypeSet += id
    
    val targetIncoming = incomingRelationships.getOrElseUpdate(targetId, mutable.Map.empty.withDefaultValue(mutable.Set.empty))
   ...
   * @return GraphResult containing the relationship or a failure
   */
   ...
   * 
   * @param id The relationship ID
   * @param properties New properties (completely replaces existing ones)
   * @return GraphResult containing the updated relationship or a failure
   */
  def updateRelationship(id: ID, properties: Properties): GraphResult[Relationship] = {
    relationships.get(id) match {
      case Some(existing) =>
        val updated = Relationship(id, existing.sourceId, existing.targetId, existing.relationType, properties)
   ...
   * 
   * @param id The relationship ID
   * @return GraphResult containing the deleted relationship or a failure
   */
   ...
   * @return Set of relationships connected to the node
   */
   ...
   * @return Set of outgoing relationships from the node
   */
  def getOutgoingRelationships(nodeId: ID, relType: Option[String] = None): GraphResult[Set[Relationship]] = {
    if (!nodes.contains(nodeId)) {
      return GraphResult.failure(s"Node $nodeId does not exist")
    }
    
    val relIds = relType match {
      case Some(t) => outgoingRelationships.getOrElse(nodeId, Map.empty).getOrElse(t, Set.empty)
      case None => outgoingRelationships.getOrElse(nodeId, Map.empty).values.flatten
    }
   ...
   * @param nodeId The node ID
   * @param relType Optional relationship type filter
   * @return Set of incoming relationships to the node
   */
  def getIncomingRelationships(nodeId: ID, relType: Option[String] = None): GraphResult[Set[Relationship]] = {
    if (!nodes.contains(nodeId)) {
      return GraphResult.failure(s"Node $nodeId does not exist")
    }
    
    val relIds = relType match {
      case Some(t) => incomingRelationships.getOrElse(nodeId, Map.empty).getOrElse(t, Set.empty)
      case None => incomingRelationships.getOrElse(nodeId, Map.empty).values.flatten
    }
   ...
   * @return Set of paths (each a list of alternating nodes and relationships)
   */
  def findPaths(startNodeId: ID, endNodeId: ID, maxDepth: Int = 10): GraphResult[List[List[Any]]] = {
   ...
   * @return Number of nodes with the given label
   */
   ...
   * 
   * @param relationType The relationship type to count
   * @return Number of relationships with the given type
   */
