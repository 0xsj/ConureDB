...
class Persistence(graph: Graph) extends LazyLogging {
  ...
  /** Parses a JSON string into serialized graph data
    *
    * Note: This is a very simplified JSON parser. In a real implementation, use a proper JSON
    * library.
    */
  ...
  private def break(): Nothing = {
    throw new RuntimeException("Loop break")
  }
}
