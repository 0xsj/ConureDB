...
object Main extends LazyLogging {
  ...
  def runDemo(graph: Graph, queryEngine: QueryEngine, persistence: Persistence): Unit = {
    ...
    val rel4 = graph.createRelationship(alice.id, projectA.id, "WORKS_ON", Map("role" -> "Lead Developer", "started" -> "2023-01-15"))
    val rel5 = graph.createRelationship(alice.id, projectB.id, "WORKS_ON", Map("role" -> "Designer", "started" -> "2023-03-10"))
    val rel6 = graph.createRelationship(charlie.id, projectA.id, "WORKS_ON", Map("role" -> "Developer", "started" -> "2023-02-01"))
    ...
    println(s"Found ${developers.size} developers: ${developers.map(_.properties("name")).mkString(", ")}")
      ...
      case Success(aliceNode) =>
        graph.getOutgoingRelationships(alice.id, Some("WORKS_ON")) match {
          case Success(rels) =>
            ...
            println(s"Alice works on ${projects.size} projects: ${projects.map(_.properties("name")).mkString(", ")}")
    ...
    println(s"After loading - Node count: ${graph.nodeCount}, Relationship count: ${graph.relationshipCount}")
   ...
   */
  private def createPerson(graph: Graph, name: String, age: Int, roles: Set[String] = Set.empty): Node = {
  ...
  private def createProject(graph: Graph, name: String, description: String): Node = {
    ...
    val properties = Map("name" -> name, "description" -> description, "created" -> java.time.LocalDate.now().toString)
    graph.createNode(labels, properties) match {
      case Success(node) => node
      case Failure(msg) => 
