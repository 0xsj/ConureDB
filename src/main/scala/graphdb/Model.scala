...
object Model {
   ...
   * @param labels Set of labels that categorize the node
   * @param properties Map of properties associated with the node
   */
  final case class Node(
    ...
    override def toString: String = 
      s"Node($id, labels=[${labels.mkString(", ")}], properties=${properties.map { case (k, v) => s"$k:$v" }.mkString("{", ", ", "}")})"
  }
   ...
   * @param relationType Type of relationship (e.g. "KNOWS", "BELONGS_TO")
   * @param properties Map of properties associated with the relationship
   */
  final case class Relationship(
    ...
    override def toString: String = 
      s"Relationship($id, $sourceId -[$relationType]-> $targetId, properties=${properties.map { case (k, v) => s"$k:$v" }.mkString("{", ", ", "}")})"
  }
